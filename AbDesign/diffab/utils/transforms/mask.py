...
class MaskSingleCDR(object):
    ...
    def __call__(self, structure):
        if self.selection is None:
            ...
            sel = None
        elif self.selection in (constants.CDR.H1, constants.CDR.H2, constants.CDR.H3, ) and structure['heavy'] is not None:
            data_to_mask = structure['heavy']
            sel = int(self.selection)
        elif self.selection in (constants.CDR.L1, constants.CDR.L2, constants.CDR.L3, ) and structure['light'] is not None:
            ...
            raise MissingChainException(f"select {self.selection}, but this chain is missing")
