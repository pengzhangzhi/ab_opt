...
class ResidueEmbedding(nn.Module):
    ...
    def __init__(self, feat_dim, max_num_atoms, max_aa_types=22,hotspot=True):
        super().__init__()
        ...
        if hotspot:
            infeat_dim += feat_dim
            self.hotspot_embed = nn.Embedding(10, feat_dim, padding_idx=0)    # 1: non-hotspot, 2: hotspot 
    ...
    def forward(
        self,
        aa, res_nb, chain_nb, pos_atoms, mask_atoms, fragment_type, 
        hotspot=None, hydropathy=None, charge=None, 
        ...
        out = torch.cat([aa_feat, crd_feat, dihed_feat, type_feat], dim=-1)
        if hotspot is not None and self.hotspot_embed is not None:
            hotspot_feat = self.hotspot_embed(hotspot)
            out = torch.cat([out, hotspot_feat], dim=-1)
        out_feat = self.mlp(out) # (N, L, F)
