...
class ProteinFeature(nn.Module):
    ...
    @torch.no_grad()
    def construct_edges(self, X, batch_id, segment_ids):
        '''
        ...
        select_edges = (row_seg == col_seg) # the src and dst node are in the same segment
        ...
        num_ctx_edge = ctx_edges.shape[-1]
        ctx_edge_feats = torch.zeros(num_ctx_edge, 1, device=X.device) # [num_ctx_edge, 1]
        
        return ctx_edges, inter_edges, ctx_edge_feats

    def forward(self, X, batch_id, segment_ids):
        """
        

        Args:
            X (torch.Tensor): [nb_all_residues, nb_atoms, 3]
            batch_id (torch.Tensor): [nb_all_residues], which sample the residue belongs to
            segment_ids (torch.Tensor): [nb_all_residues], which segment the residue belongs to
            cdr_range (torch.Tensor): [bs, 2]

        Returns:
            ctx_edges, edges within the same segment
            inter_edges, egdes between different segments
            ctx_edge_feats, [num_ctx_edge, 1] initialized to 0
        """
        
        ctx_edges, inter_edges, ctx_edge_feats = self.construct_edges(X,batch_id,segment_ids)
        return ctx_edges, inter_edges, ctx_edge_feats
...
class EfficientMCAttModel(MCAttModel):
    ...
    def forward(self, X, S, L, offsets):
        ...
        # init mask
        # mask in this code context means that the invalid positions denoted by 1.
...
def interpolate(X, cdr_range):
    '''
    set coordinates of masks following a unified distribution
    between the two ends
    '''
    X, cmask = X.clone(),  torch.zeros_like(X, device=X.device)
    n_channel, n_dim = X.shape[1:]
    for start, end in cdr_range:
        l_coord, r_coord = X[start - 1], X[end + 1]  # [n_channel, 3]
        n_span = end - start + 2
