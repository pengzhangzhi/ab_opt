...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', type=str, default='configs/train/dock_antibody.yml')
    ...
    writer = BlackHole()
    if os.environ.get('wandb',False):
        import wandb
        wandb.init(project='diffab_dock', config=dict(config), name=f"{args.tag}")
    # Logging
    if args.log == False:
    ...
    if is_resume or is_finetune:
        ckpt_path = args.resume if is_resume else args.finetune
        logger.info('Resuming from checkpoint: %s' % ckpt_path)
        ckpt = torch.load(ckpt_path, map_location=args.device)
        it_first = ckpt['iteration']  if is_resume else 1
        msg = model.load_state_dict(ckpt['model'], strict=is_resume)
        logger.info(msg)
        if is_resume:
