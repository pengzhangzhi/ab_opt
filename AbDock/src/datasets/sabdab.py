...
from src.utils.transforms.patch import DISTVIOLATION
...
def _label_whole_chain_as_cdr(data, seq_map, max_chain_length=120):
    """ label the whole chain to be CDRH3."""
    assert len(data['aa']) <= max_chain_length, f'Chain too long {len(data["aa"])}'
    if data is None or seq_map is None:
        return data, seq_map
    data['cdr_flag'] = torch.ones_like(data['aa']).fill_(constants.CDR.H3)
    return data, seq_map
...
class SAbDabDataset(Dataset):
    ...
    def _load_split(self, split, split_seed):
        assert split in ('train', 'val', 'test','dock_test')
        ids_test = [
            entry['id']
            for entry in self.sabdab_entries
        ...
        ids_docktest = [
            entry['id']
            for entry in self.sabdab_entries
            if entry['id'] in DOCK_TESTSETS
        ]

        dock_test_relevant_clusters = set([self.id_to_cluster[id] for id in ids_docktest])
        ids_train_val = [
            entry['id']
            for entry in self.sabdab_entries
            if (self.id_to_cluster[entry['id']] not in test_relevant_clusters) and \
                (self.id_to_cluster[entry['id']] not in dock_test_relevant_clusters)
        ]
        random.Random(split_seed).shuffle(ids_train_val)
        if split == 'test':
            self.ids_in_split = ids_test
        elif split == 'dock_test':
            self.ids_in_split = ids_docktest
            # print(self.ids_in_split)
        elif split == 'val':
    ...
    def __getitem__(self, index):
        ...
        if self.must_have_antigen and data.get('antigen', None) is None:
            return self.__getitem__((index + 1) % len(self))
        if self.transform is not None:
            try:
                data = self.transform(data)
            except DISTVIOLATION as e:
                return self.__getitem__((index + 1) % len(self))
        return data
