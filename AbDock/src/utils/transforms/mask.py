...
@register_transform('mask_full_antibody')
class MaskFullAntibody(object):
    def __init__(self, antibody_chains: Optional[List[str]]=['heavy']) -> None:
        self.antibody_chain2enum = {
            'heavy': constants.Fragment.Heavy,
            'light': constants.Fragment.Light,
        }
        self.selections = [self.antibody_chain2enum[chain] for chain in antibody_chains]
    
    def maskone_chain_(self, data):
        data['generate_flag'] = torch.ones(data['aa'].shape, dtype=torch.bool)
        anchor_flag = torch.zeros(data['aa'].shape, dtype=torch.bool)
        anchor_flag[0] = True
        anchor_flag[data['aa'].size(0)-1] = True
        data['anchor_flag'] = anchor_flag
        
    def __call__(self, structure):
        """
        for selected antibody chains:
            - set the whole chain to be generated.
            - set the first and last residue to be anchors.
        """
        if (structure['heavy'] is not None) and \
            (constants.Fragment.Heavy in self.selections):
            self.maskone_chain_(structure['heavy'])
            
        if (structure['light'] is not None) and \
            (constants.Fragment.Light in self.selections):
            self.maskone_chain_(structure['light'])
        return structure
